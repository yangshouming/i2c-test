...
#define I2C_SMBUS_I2C_BLOCK_MAX	2048	/* Not specified but we use same structure */
...
union i2c_smbus_data
{
  ...
  uint16_t word ;
  uint32_t dword ;
  uint8_t  block [I2C_SMBUS_I2C_BLOCK_MAX + 2] ;	// block [0] is used for length + one more for PEC
} ;
...
/*
 * mywiringPiI2CReadReg8: mywiringPiI2CReadReg16:
 *	Read an 8 or 16-bit value from a regsiter on the device
 *********************************************************************************
 */

int mywiringPiI2CReadReg8 (int fd, int reg)
{
  union i2c_smbus_data data;
...
int mywiringPiI2CReadReg16 (int fd, int reg)
{
  union i2c_smbus_data data;
...
int mywiringPiI2CWrite (int fd, int data)
{
...
/*
 * mywiringPiI2CWriteReg8: mywiringPiI2CWriteReg16:
 *	Write an 8 or 16-bit value to the given register
 *********************************************************************************
 */

int mywiringPiI2CWriteReg8 (int fd, int reg, int value)
{
  union i2c_smbus_data data ;
...
int mywiringPiI2CWriteReg16 (int fd, int reg, int value)
{
  union i2c_smbus_data data ;

  data.word = value ;

  printf("data.block %x %x %x %x\n",data.block[0],data.block[1],data.block[2],data.block[3]);
...
*/
int mywiringPiI2CReadReg16_mlx (int fd, uint16_t reg, uint16_t *value,uint16_t length)
{
...
//    int i;
//
//    data.block[0] = length;//block[0] 保存长度(byte)
//
//    printf("data.block start %x %x %x %x\n",data.block[0],data.block[1],data.block[2],data.block[3]);
//
//
//    ret=i2c_smbus_access(fd,I2C_SMBUS_READ,reg>>8,I2C_SMBUS_I2C_BLOCK_DATA, &data); 
//
//    
//    printf("data.block end %x %x %x %x\n",data.block[0],data.block[1],data.block[2],data.block[3]);
//
...
int mywiringPiI2CWriteReg16_mlx (int fd, int reg, int value)
{
    union i2c_smbus_data data; 

    data.block[0] = 3;//block[0] 保存长度
    data.block[1] = reg>>0; 
    data.block[2] = value>>8; 
    data.block[3] = value>>0; 
    
    printf("data.block %x %x %x %x\n",data.block[0],data.block[1],data.block[2],data.block[3]);

 
    return i2c_smbus_access(fd,I2C_SMBUS_WRITE,reg>>8,I2C_SMBUS_I2C_BLOCK_DATA, &data); 

}


/*
 * mywiringPiI2CSetupInterface:
 *	Undocumented access to set the interface explicitly - might be used
 *	for the Pi's 2nd I2C interface...
 *********************************************************************************
 */

int mywiringPiI2CSetupInterface (const char *device, int devId)
{
  int fd ;
...
int mywiringPiI2CSetup (const int devId)
{
  int rev ;
  const char *device ;
  ...
  return mywiringPiI2CSetupInterface (device, devId) ;
}
